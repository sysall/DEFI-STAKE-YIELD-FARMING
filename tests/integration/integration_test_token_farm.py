...
from scripts.deploy import deploy_token_farm_and_sunucash_token
import pytest
...
def test_stake_and_issue_correct_amounts(amount_staked):
    ...
    account = get_account()
    sunucash_token.approve(token_farm.address, amount_staked, {"from": account})
    token_farm.stakeTokens(amount_staked, sunucash_token.address, {"from": account})
    ...
    # Assert
    assert (
        sunucash_token.balanceOf(account.address)
